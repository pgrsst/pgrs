...
use pgrs_core::TlsMode;
use pgrs_core::{AddConnectionInput, ConnectionApi, EditConnectionInput};
...
impl ConnectionHandler {
    pub fn new(api: ConnectionApi) -> Self {
        Self { api }
    }
    ...
    pub fn add(&self, args: &[String]) -> Result<(), String> {
        ...
        let port = optional_option(args, "--port")
            ...
            .transpose()?
            .or(url.port)
            .unwrap_or(pgrs_core::DEFAULT_PORT);
        ...
        self.api.add(AddConnectionInput {
            name: name.clone(),
            host,
            port,
            username,
    ...
    pub fn edit(&self, args: &[String]) -> Result<(), String> {
        ...
        let resolved_name = self.api.find(&name)?.name.clone();
        self.api.edit(&resolved_name, EditConnectionInput {
    ...
    pub fn rename(&self, args: &[String]) -> Result<(), String> {
        ...
        let new_name = args
            ...
            .trim()
            .to_string();
        let resolved_old = self.api.find(&old_name)?.name.clone();
        self.api.rename(&resolved_old, &new_name)?;
    ...
    pub fn connect(&self, args: &[String]) -> Result<(), String> {
        ...
        let connection = self.api.find(&name)?;
    ...
    pub(crate) fn get_connection(
        &self,
        name: &str,
    ) -> Result<pgrs_core::Connection, String> {
        self.api.get(name).map_err(|e| e.to_string())
    }
}
...
mod tests {
    ...
    fn handler_with(names: &[&str]) -> ConnectionHandler {
        ConnectionHandler::new(ConnectionApi::in_memory_with(names))
    }
